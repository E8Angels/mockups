...
| D-3 | routes/data-query.js:64 | `send_confirm_requests` action unregistered until the confirm-link email flow exists | Task 2.3 | open |
| D-4 | routes/data-query.js:65 | `run_estimates` action unregistered until the estimation engine exists | Task 3.4 | open |
| D-5 | docs/data-query-skill-portfolio-module.md | Merge the portfolio module into the external `E8Angels/data-query-skill` repo (at ~/dev/data-query-skill) — hold until the portfolio API is deployed to prod and a write token is issued | After 1.8 + deploy | open |
