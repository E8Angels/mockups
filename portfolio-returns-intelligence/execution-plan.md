...
| ID | Task | Model | Depends on | Verification | Status |
|----|------|-------|-----------|--------------|--------|
| 6.1 | ✅ 7159b7d5 — **Engine company-axis rollup**: add a `byCompany` grouping in `computeOrgPortfolioEstimates` (mirror the existing `byPerson` loop, reuse `aggregatePositions` unchanged) → per-company {invested, realized band, unrealized band, totalValue band, moic band, irr, coverage.byTier, positionCount=members, bySource}; derive DPI (realized.expected/invested), markup (totalValue.expected−invested), memberCount (distinct person), dominant/weakest tier, vintage (min firstInvestmentDate). New cache-manager method `computeCompanyPortfolioEstimates` (or extend org). SWR-cached like org. | opus | — | unit tests: company rollup ≡ sum of its member positions; DPI/markup/vintage correct; bands RSS-combine; cached | ✅ (axis reconciles; `computeCompanyPortfolioEstimates` + shared `_computeOrgEstimateBundle`; lastMarkDate deferred → D-6.1a) |
| 6.2 | *(bundled with 6.3 — same island/route/data)* **Enriched summary payload + committee dashboard header**: stop discarding realized/unrealized, IRR, MOIC band, coverage.byTier, bySource in `buildAdminPortfolioSummaryPayload`; build the header dashboard — headline TVPI + DPI + IRR (each with range), realized-vs-unrealized split, coverage/confidence composition bar, active/exited/closed composition; as-of date picker + assumption-set selector (Default/Conservative). | opus (API) + sonnet (UI from the design) | 6.1 or independent | route test + smoke ✅ | ✅ 8ae83d5b (6.2+6.3 bundled; header multiples+ranges+coverage+controls) |
| 6.3 | *(bundled with 6.2)* **Grid rows on the engine**: back the portfolio grid rows with 6.1 (add fields est_value_low/high, confidence_tier, realized, dpi, markup, exit_date, exit_type, member_count, vintage to `lib/admin-grid-config.js` portfolio config + populate in the row builder from the engine, keeping marks only where genuinely needed); `renderCell` for value ranges, tier badges, green/red markup, stale-mark flag; markup as a `formula` field so it sorts/exports. Exited companies must now show realized value, not $0. | opus | 6.1 | exited shows realized≠$0; engine-backed grid rows; D-6.1a closed | ✅ 8ae83d5b |
| 6.4 | **Seeded per-status views**: add portfolio view seeds to `scripts/seed-admin-grid-default-views.js` — **Exited** (Company, Vintage, Invested, Realized, DPI, Exit date/type, Confidence, residual), **Active** (Invested, Est Value range, MOIC range, markup/markdown, last-mark + staleness, tier), **Closed/Written off** (Invested lost, close date, residual), **Needs attention** (rank by capital × confidence weakness = the D-10 worklist), **All grouped by status** with subtotals; portfolio `defaultSummaryTypes`. Dev seed; prod seed is gated + deploy field code first (is_empty-matches-all gotcha). | sonnet | 6.3 | dev-seeded ids 84-89, well-formed (30/30 tests); prod gated | ✅ 965ba651 (Exited/Active/Closed/Needs-attention/All-by-status/By-vintage) |
| 6.5 | **Sibling charts**: realized-vs-unrealized stacked bar, coverage composition bar, vintage-cohort table (invested/value/DPI/TVPI by year), value-over-time line from `position_estimate_snapshots` (quarterly). Sibling components beside/above the grid, fed by 6.1/6.2 engine output. | sonnet | 6.2 | charts render real dev data (2016 cohort 4.65x TVPI); value-over-time empty-state (0 dev snapshots) | ✅ 9adf020d (recharts + hand-rolled bars; PortfolioInsightStrip) |
| 6.6 | **Verify + code-review + PR** — full portfolio/admin-grid/island battery green, Vite build clean, integrated browser smoke passed on worktree, medium code-review sweep completed and confirmed fixes pushed to existing PR #440. Review fixes: as-of/assumption runtime controls now drive portfolio grid rows/filter-options/hydration/export, Phase-6 bars use SVG geometry with no inline styles, and Portfolio Committee follow-ups moved the health stats/charts/vintage cohorts/documents into the committee dashboard layout. Post-deploy (gated): prod-seed the 6 views via `node scripts/seed-admin-grid-default-views.js --env=prod --apply` AFTER the field code deploys. No DB schema change in Phase 6. | opus | 6.3–6.5 | 35 suites / 475 tests; smoke; review; PR #440 merged to main (`148508be`) | ✅ merged 2026-07-06 |
...
| D-6.1a | ~~computeCompanyPortfolioEstimates~~ | lastMarkDate + markStale | Task 6.3 | **closed 8ae83d5b** (mark.effectiveDate plumbed onto estimate; staleness derived) |
...
| 2026-07-04 | **Phase 5 complete and LIVE**: PR #438 squash-merged (4c60ebf0), prod claims table rebuilt pre-deploy, v1288 deployed and verified (all unified endpoints gated correctly; log errors were Fly deploy-time machine-API noise). One recording system: every door — admin dialog (both modes), member Adjust Valuation, prose intake, agent actions — flows source→claim→event through one engine. |
| 2026-07-04 | PR #438 code review (5 finder angles): 8 confirmed findings fixed in ecea747c — incl. a CRITICAL blank-conversion-multiple→$0 money bug (empty-string guard lost in builder extraction; parity suite gap closed) and a member-private→global scope-enforcement hole (fixed at depth: materialization_scope persisted on claims, enforced in applyPortfolioClaim). Agent surface added to the parity suite (scenario 9, 10/10). |
| 2026-07-06 | **Phase 6 complete in PR #440**: final verify passed (portfolio/admin-grid/island battery 35 suites / 475 tests, Vite build, integrated browser smoke on worktree). Code-review sweep fixed stale grid/export runtime controls for as-of/assumption changes and removed inline styles from the new stacked bars. No DB schema change; prod view seed remains gated until after deploy: `node scripts/seed-admin-grid-default-views.js --env=prod --apply`. |
| 2026-07-06 | PR #440 review follow-up: moved the Phase-6 portfolio health stats and insight strip from the admin Portfolio management tab to the Portfolio Committee dashboard; added info tooltips and company drilldown drawers for stats, chart segments, and Vintage Cohort company counts. |
| 2026-07-06 | PR #440 merged to `main` at `148508be` with final committee-dashboard follow-ups: Documents in the right panel under Upcoming Meetings and Vintage Cohorts sorted newest-first. No Phase 6 migrations; prod saved-view seed remains pending until after the deploy. |
