...
| 6.6 | **Verify + code-review + PR** — REMAINING (handoff point). Branch `portfolio-committee-view` pushed to origin, based on current origin/main, Phase-6 suites 54/54 green, vite build clean, TODO sweep clean. TODO: full battery, integrated browser smoke of the whole page (header + grid + the 6 seeded views + insight strip), medium code-review sweep of `git diff origin/main...HEAD` (money+UI — caught real bugs on #410/#438; fix confirmed findings), open PR to main. Post-deploy (gated): prod-seed the 6 views via `node scripts/seed-admin-grid-default-views.js --env=prod --apply` AFTER the field code deploys. No DB schema change in Phase 6. | opus | 6.3–6.5 | battery green; smoke; review; PR | ⬜ HANDOFF |
